"""
DWE Airflow Infrastructure — AWS Pulumi IaC
...
# ── Airflow runtime — validate presence; EC2 reads them from .env at boot ─────
...
# ─────────────────────────────────────────────────────────────────────────────
# User data — install Docker, clone repo, write .env, start Airflow
# ─────────────────────────────────────────────────────────────────────────────
...
lt = aws.ec2.LaunchTemplate(
    ...
    tag_specifications=[aws.ec2.LaunchTemplateTagSpecificationArgs(
        ...
        tags={**tags, "Name": f"airflow{suffix}"},
    )],
    tags=tags,
)
...
# ─────────────────────────────────────────────────────────────────────────────
# Auto Scaling Group
# Airflow is stateful (DAGs/logs on disk) — single instance recommended.
# ─────────────────────────────────────────────────────────────────────────────
asg = aws.autoscaling.Group(
    ...
    tags=[aws.autoscaling.GroupTagArgs(key=k, value=v, propagate_at_launch=True) for k, v in {**tags, "Name": f"airflow{suffix}"}.items()],
)
